...
void BoundingSphere::UpdootPosition(vector3 v3Input)
{
	this->center = v3Input;
}
...
void BoundingSphere::calcRadius()
{
	...
	for (uint i = 0; i < vertexList.size(); i++)
	{
		...
		center = (max + min) / 2.0f;
		//center *= vector3(vector4(center, 0.0f) * glm::translate(matrix4(IDENTITY), this->position));
		radius = 0;
...
void BoundingSphere::calcCenter(vector3 pos){
